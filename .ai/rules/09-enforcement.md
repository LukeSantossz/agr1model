...
- **Configurável:** Padrões de debug log (`console.log`, `print`, `debugger`) são definidos em um arquivo `.ai/enforcement.conf` que lista os patterns por linguagem. O hook lê este arquivo se existir; caso contrário, usa um conjunto padrão.
...
Após a instalação, o diretório `.ai/hooks/` contém os scripts e o comando `git config core.hooksPath .ai/hooks` redireciona o git para usá-los. Isso garante que os hooks são versionados junto ao repositório e compartilhados entre todos os desenvolvedores do projeto.
